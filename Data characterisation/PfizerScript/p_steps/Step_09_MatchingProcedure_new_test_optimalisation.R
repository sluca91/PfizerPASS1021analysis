...
if(MATCHING_METHOD == "LOOP") warning("Optimalisation only tested for sql")
...
if(MATCHING_METHOD == "SQL"){
  ...
  if(parallel_method == "NONE"){
    ...
    MATCHING_SQL_spells(print = T, db = mydb, colls =  "status", ids = paste0(x, collapse = ","))})
...
TEMP2 <- dbGetQuery(mydb ,paste0("SELECT DISTINCT person_id, FIRST_PFIZER AS T0, ST,EN, status, YEAR_BIRTH FROM Exposed t1"))
TEMP3 <- dbGetQuery(mydb ,paste0("SELECT DISTINCT * FROM id_table"))
dbDisconnect(mydb)
...
MATCHED <- as.data.table(sqldf(paste0("SELECT DISTINCT 
              ...
              LEFT JOIN TEMP3 t3 on(t1.status = t3.status)
...
saveRDS(MATCHED,"C:/Users/relbers/Documents/GitHub/Pfizer/Data characterisation/PfizerScript/g_intermediate/populations/MATCH_PAIRS_test_optimalisation.rds")

rm(Dictionary, mydb, files, SCRIPT, MATCHED)
gc()
