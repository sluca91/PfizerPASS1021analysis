...
#Work in progress
#This function is build up again based on the scrip Step_12_AddCoVariates_new2. Only prior = T is in. The rest is deleted to be sure that it is not used.
#Copy over from GetDatesIR to add also to other quries. Note that the group by and select needs to be changed for every query.  


GetDatesIR2 <- function(
  ...
  if(prior){
      ...
      if(prior.sum == F){
         temp1 <- as.data.table(dbGetQuery(mydb,paste0(
           ...
           SELECT person_id, Date, id, concept, cohort ",coll0, start0,"  FROM(
           SELECT
           * ,
           ROW_NUMBER () OVER (
                           PARTITION BY person_id, id, concept, cohort
