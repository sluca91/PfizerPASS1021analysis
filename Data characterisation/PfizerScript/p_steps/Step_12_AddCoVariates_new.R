...
if(length(Available_cov) > 0){
  ...
  #Put the extraction process in a function that can be used for parallel and non parallel extraction
  ApplyExtraction <- function(x){ 
    
    library("DBI")
    library("data.table")
    library("RSQLite")
    
    #Open a temporary database (per core) to append extracted information to. It is easier to append to a database
    mydb <- dbConnect(RSQLite::SQLite(), "")  
    
    #Per row in scheme extract the dates and put it in a standardized file format that can be appended
    for(i in x){  
      
      #Get the dates via the function GetDatesIR in which they needed SQL queries are stored
      COV2 <-   GetDatesIR(
      ...
      COV2 <- ProcessOutputGetIRDates(COV2 = COV2, i = i)
      
      #Write the extracted dates to a long file
      dbWriteTable(mydb, "fileresult", COV2$result, append = T)
      
      #Some concepts have additional information that is needed later on. This is stored in a separate file so columns with lots of NA's are prevented
      #The file with this information can also be removed for later versions because it does not add much anymore since the units are handled earlier in the
      #script
      if(!is.null(COV2$voc)) dbWriteTable(mydb, "filevoc", COV2$voc, append = T)
      
      rm(COV2)
      
    }
    
    #when all concepts are process get the appended information and store in a list suited for parallelism. 
    if("filevoc" %in% dbListTables(mydb)){filevoc <- as.data.table(dbReadTable(mydb, "filevoc"))}else{filevoc <- NULL}
    fileresult <- as.data.table(dbReadTable(mydb, "fileresult"))
    dbDisconnect(mydb)
    
    return(list(result = fileresult, voc = filevoc))  
  ...
  if(parallel_method %in% c("WINDOWS")) {
    ...
    #Divide the rows in scheme in groups. Every group is the input for 1 core
    parallelGroups <- 1:nrow(scheme)
    parallelGroups <- split(parallelGroups, sort(seq_along(parallelGroups))%%(nb_cores))
    
    #Set up the cluster 
    clust <- makeCluster(nb_cores, setup_timeout = 5000, outfile=paste0(populations_dir,"log3.txt"))
    clusterExport(clust, varlist =  c("mydb","tmp","GetDatesIR","scheme", "dbconcepts", "ProcessOutputGetIRDates", "parallelGroups", "ApplyExtraction"))
      
    COV3 <- parLapply(cl = clust, parallelGroups, function(x) ApplyExtraction(x))
    stopCluster(clust)
    rm(clust, parallelGroups)
    }
  
  if(parallel_method %in% c("NONE")) {
    COV3 <- lapply(list("0" = 1:nrow(scheme)), function(x) ApplyExtraction(x))
    
  }
    
  if(exists("fileresult")) rm(fileresult)
  fileresult <- NULL
  for(i in 1:length(COV3)){fileresult <- rbind(fileresult,COV3[[i]]$result) }
  
  if(exists("filevoc")) rm(filevoc)
  filevoc <- NULL
  for(i in 1:length(COV3)){filevoc <- rbind(filevoc,COV3[[i]]$voc) }
