...
check0 <- as.data.table(temp)[person_id != "#ID-00009727#",][, old_op_start_date := as.Date(old_op_start_date, origin = "1970-01-01")][, old_op_end_date := as.Date(old_op_end_date, origin = "1970-01-01")]
...
check1 <- check[,.(L_SEX_COV.x,L_SEX_COV.y)][L_SEX_COV.x == L_SEX_COV.y, x := "correct"]
...
COV <- readRDS(paste0(concepts_dir,"I_COVID_COV.rds"))
...
INF <- readRDS(paste0(concepts_dir,"INF.rds"))
...
if(exists("TEST2")){
  TEST2 <- readRDS(paste0(populations_dir,"MATCH_PAIRS.rds"))[!is.na(Control),]
...
REG <- readRDS(paste0(concepts_dir,"L_GEOREGION_COV.rds"))
...
check7 <- merge(check7a[, .(Exposed, Control, Value)],check7b[, .(Exposed, Control, Value)], by = c("Exposed","Control"), allow.cartesian = T)[Value.x == Value.y, x := "correct"]


check7 <- unique(check7[ x == "correct",])

check7 <- merge(TEST[,.(Exposed, L_GEOREGION_COV, nb_match)],check7, by = "Exposed", all.x = T)
#check7_not_min <- check7
check7 <- check7[L_GEOREGION_COV == "MISSING" & is.na(Value.x) & is.na(Value.y) ,x := "correct",]

check7 <- check7[L_GEOREGION_COV != "MISSING" & nb_match == 0,x := "correct"]
