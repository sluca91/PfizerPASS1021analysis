...
ProcessOutputGetIRDates <- function(x){
...
#Go over the list. The list contains data frame's that need to be combined to 1 long file
  ...
  if(exists("file1", where = COV2[[i]])){
    ...
    if("Unit" %in% colnames(temp2)){
      ...
      dbWriteTable(mydb, "filevoc", tempvoc, append = T)
    }  
    ...
    #Append
    dbWriteTable(mydb, "fileresult", temp2, append = T)
...
x <- 1:length(COV2)
x <- split(x, sort(seq_along(x))%%(nb_cores))

clust <- makeCluster(nb_cores, setup_timeout = 5000)
clusterExport(clust, varlist = c("COV2", "scheme")  )

COV3 <- parLapply(cl = clust, x, ProcessOutputGetIRDates)

stopCluster(clust)
rm(clust)

if(exists("fileresultCompare")) rm(fileresultCompare)
fileresultCompare <- NULL
for(i in 1:length(COV3)){fileresultCompare <- rbind(fileresultCompare,COV3[[i]]$result) }

if(exists("filevocCompare")) rm(filevocCompare)
filevocCompare <- NULL
for(i in 1:length(COV3)){filevocCompare <- rbind(filevocCompare,COV3[[i]]$voc) }
...
#as.Date(as.integer(0), origin = "1970-01-01")
#as.Date(NA)
