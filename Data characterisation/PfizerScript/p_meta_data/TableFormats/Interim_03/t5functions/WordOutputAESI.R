...
for(i in 1:length(x)){
  ...
  formatNew <- fread(paste0(projectFolder,"/Format", x[i], "Target.csv"))
  ...
  for(j in 1:length(aesi)){
   ...
   if (x[i] == "Table20") {
    ...
    justTitle <- sprintf(justTitle,aesi[j])
    justTitle <- str_replace(justTitle,'percent ','%')
    
    ft <- CreateFlextable(formatNew, tableTmp, justTitle)
    
    CreateAndPrint_AESI_ToWordDocument(projectFolder, ft, filename, tabletitle, orient = 'landscape', InterMediateFolder, tablelist = TableList, TableNr[i], 1)
